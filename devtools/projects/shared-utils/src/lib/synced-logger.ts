...
export interface SyncedLoggerEvents {
  ...
  [SYNC_LOGS_MSG]: (logs: Map<string, SyncedLog>) => void;
  [SYNC_REQUEST_MSG]: () => void;
}
...
export class SyncedLogger {
  ...
  addChannel(messageBus: MessageBus<unknown>) {
    ...
    bus.on(SYNC_LOGS_MSG, (extLogs) => {
      for (const [id, log] of Array.from(extLogs)) {
  ...
  exportFullLog() {
    ...
    downloadLink.href = URL.createObjectURL(
      new Blob([JSON.stringify(this.getFullLog())], {type: 'application/json'}),
    );
    downloadLink.click();
  ...
  private emitAppData(
    ...
    if (Array.isArray(channels) ? !channels.length : !channels?.size) {
      console.warn('SyncedLogger: There are no channels to notify.');
    }
  ...
  private emitLogs(
    ...
    for (const bus of channels) {
      bus.emit(SYNC_LOGS_MSG, [this.logs]);
    }
  }
}
